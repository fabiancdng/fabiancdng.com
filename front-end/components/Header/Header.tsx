...
const Header = ({ data, statics }: headerProps) => {
    ...
    return (
        <header className="fixed top-0 left-0 w-full z-20 bg-white dark:bg-slate-800">
            ...
            {/* Mobile menu */}
            { mobileMenuOpen && <ul className="p-4 mt-2 block space-y-3 md:hidden bg-slate-100 dark:bg-slate-700">

                {
                    ...
                    data.links.data?.map((link, index) => (
                        ...
                        >
                            <HeaderLink data={ link } additionalCSS="block w-full px-3 py-2" />
                        </li>
                    ))
                }
            </ul> }
